...
{ 
    static function createTempTable($table,$file,$postavschik){ // Записываем данные из файла во временную таблицу
      // Не забыть Sys::deleteEmptyFiles($dir);
        ...
        while (($values = fgetcsv($fd, 262144, ",")) !== FALSE)                        
        {                                                        
/*
...
$filtr = "";
$filter_connect = "";
include SITE_PATH."/cms/price/erc_filter_id.php";
$category_id = "";				
include SITE_PATH."/cms/price/erc_category_id.php";
...
12 - Наличие	
13 - Ожидается	
14 - Участвует в акции	
15 - Условия акции	
16 - В упаковке	
17 - Товаров со скидкой (повр. упакаковка)	
18 - Описание	
19 - Комплектация	
20 - Доп.Инфо.
*/
...
// Наличие
if(isset($values[11]) && $values[11] !== ""){    // Склад
    if($values[11] == "Есть"){$nalichie = "+";} 
    elseif($values[11] == "Нет"){$nalichie = "-";}
   // elseif($values[11] == "Выписано"){$nalichie = "Выписано";}
    else{$nalichie = "r";}
}
if(isset($values[12]) && $values[12] !== ""){  //Наличие (not use)
    $ostatki = $values[12];
}
// /Наличие

//$values[18] = preg_replace('#<a href=(?:.*)(?=</a>)#Usi', '', $values[18]);//Описание - удаляем все ссылки 
if(isset($values[18]) && $values[18] !== ""){     //Описание
    $description_full = strip_tags(addslashes($values[18]));
    //$description_full = strip_tags($values[18]);
    //$description_full = ereg_replace("&bull;","<br />",$description_full);
}
else{
	$description_full = "";
}
if(isset($values[19]) && $values[19] !== ""){ // Краткое описание (Комплектация)
    $description_short = strip_tags(addslashes($values[19]));
    //$description_short = ereg_replace("&bull;","<br />",$description_short);
}
else{
	$description_short = "";
}
if(isset($values[14]) && $values[14] !== ""){ // Участвует в акции
    $note_1 = $values[14];
}
if(isset($values[15]) && $values[15] !== ""){ // Условия акции
    $note_2 = $values[15];
}
