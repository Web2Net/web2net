...
if($event!=""){
$tag_data=MessageTag::tagData($id);

if($tag_data["level"]==1){
MessageTag::Event1($event,MESSAGE_TAG_TABLE,$id);
}
MessageTag::Event2($event,MESSAGE_TAG_TABLE,$id);
}
	
$tag1_list=MessageTag::tagListIdData($level=1);
$tag2_list=MessageTag::tagListIdData($level=2);
...
if($arr_value['level']==2){
	$tag_connect_list=MessageTag::tagListConnectData($id);
    ...
    $c_tag_data=MessageTag::tagData($val);
...
if($arr_value['level']==2){	
	...
	rename(MESSAGE_ITEM_IMG_PATH."/new",MESSAGE_ITEM_IMG_PATH."/".$id);
...
/*
if($event!=""){FotoMessage::Event($event,'items_branch',$id);}
if($com=="add"){
...
$cat_data=FotoMessage::catData($id);
