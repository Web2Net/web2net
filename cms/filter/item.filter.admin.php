...
if($event!=""){
FilterItem::Event($event,FILTER_ITEM_TABLE,$id,$tag);
}

$item_list=FilterItem::itemListIdData($tag);

if(INI::Life('filter_tag_list')>100){
$tag_list=FilterTag::tagListIdData();
INI::SetXXL($tag_list,'filter_tag_list');
}
$tag_list=INI::Get('filter_tag_list');
...
$table=FILTER_ITEM_TABLE;
$table2=FILTER_ITEM_TABLE_LANG;
...
if($site_langs !== NULL){   
    foreach($site_langs as $key=>$val){   
	    if($val['lang']==$_SESSION['lang']){
	        ...
	        $table_lang="filter_item_".$val['lang'];
	        $arr_value3['pid']=$id;
...
$tag_list1=FilterTag::tagListIdData(1);
$tpl_item->assign('tag_list1',$tag_list1);

$tag_list2=FilterTag::tagListIdData(2);
$tpl_item->assign('tag_list2',$tag_list2);

if($com=="edit"){
$item_data=FilterItem::itemData($id);
...
SYS::varDump($arrcat,__FILE__,__LINE__,"FILTER::itemListIdData");
return $arrcat;
*/
}

static function itemShowListIdData($tag){
...
SYS::varDump($row,__FILE__,__LINE__,"FILTER::itemData");
return $row[0];
*/
}
